...
export function registerWebhookRoutes(app: FastifyInstance, ctx: AppContext): void {
  app.post("/webhooks/jira", async (request, reply) => {
    ...
    log.info(
      { step: "webhook.received", summary: payload.summary, issueType: payload.issueType, phase: payload.phase },
    ...
    if (!shouldAnalyze) {
      log.info(
        { step: "webhook.ignored", state: ticket.state },
        "trigger ignored: ticket already in progress or done",
      );
    ...
    log.info({ step: "webhook.queued", state: ticket.state }, "Phase A queued (RECEIVED → will ANALYZE)");
