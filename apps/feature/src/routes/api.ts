  ...
  app.register(async (api) => {
    api.addHook("onRequest", async (request, reply) => {
      if (!secretMatches(request.headers["x-api-key"] as string | undefined, ctx.env.API_KEY)) {
        ctx.logger.warn({ step: "api.auth_failed", path: request.url }, "API request rejected: invalid X-Api-Key");
    ...
    api.post<{ Params: { issueKey: string } }>("/api/tickets/:issueKey/approve", async (request, reply) => {
      ...
      await ctx.audit.record(issueKey, "implement", "approval_received", body.data.notes);
      log.info({ step: "approve.queued", state: "IMPLEMENTING" }, "Phase B queued (AWAITING_APPROVAL → IMPLEMENTING)");
      ctx.queue.push(`phaseB:${issueKey}`, () => runPhaseB(ctx, issueKey));
      return reply.code(202).send({ status: "implement_queued", issueKey });
    });
