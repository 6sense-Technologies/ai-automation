...
import { RepoAccessError } from "../errors.js";
import { REPORT_DIR } from "../providers/types.js";
...
export class RepoManager {
   ...
   * local state). Used before Phase A and as the base for new fix branches.
   */
  ...
  /**
   * Read-only guard for Phase A: report any modification the agent made and
   * revert it. Returns the list of dirtied paths (empty when clean).
   */
  ...
  /** Find an existing remote bugfix branch for this ticket (idempotency / resume). */
  ...
  /** Create (or resume) the bugfix branch. Returns true when resumed from remote. */
  ...
  /** Push the bugfix branch. This is the only write this service ever makes to the remote. */
