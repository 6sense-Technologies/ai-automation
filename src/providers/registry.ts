 ...
 * ...), implement AgentProvider (see types.ts) and add a factory here; select
 * it via `defaultProvider` or a per-project `provider` in pipeline.config.yaml.
 */
const factories: Record<string, ProviderFactory> = {
  ...
  // "claude-code": (env, config) => new ClaudeCodeProvider({ ... }),
  // "opencode": (env, config) => new OpenCodeProvider({ ... }),
};
